...
def create_borrower(first_name: str, last_name: str, email: str = None,
    ...
    row = d1_one(
        ...
        RETURNING *
        """,
        [first_name, last_name, email, phone_number, relationship_type, address],
    )
    return dict(row)

def get_borrower_by_id(person_id: int) -> Optional[Dict[str, Any]]:
    """Busca um mutuário específico pelo ID."""
    row = d1_one("SELECT * FROM borrowers WHERE person_id = ?", [person_id])
...
def set_borrower_status(person_id: int, new_status: str) -> Dict[str, Any]:
    ...
    row = d1_one(
        "UPDATE borrowers SET status = ? WHERE person_id = ? RETURNING *",
        [new_status.upper(), person_id],
    )
    if row is None:
